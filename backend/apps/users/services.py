...
class StreakService:
    @staticmethod
    def get_streak_data(user: User, tz_name: str = "America/Sao_Paulo") -> StreakDataOut:
    ...
    @classmethod
    def record_activity(cls, user: User, tz_name: str = "America/Sao_Paulo") -> StreakRecordOut:
        streak_data = user.streak_data if isinstance(user.streak_data, dict) else {}
        today = _get_local_today(tz_name or user.timezone)
    ...
    def purchase_freeze(cls, user: User) -> PurchaseFreezeOut:
        ...
        if freeze_count >= 3:
            raise HttpError(400, "Você já atingiu o limite máximo de 3 proteções de Streak Freeze.")

        if user.total_xp < 150:
            raise HttpError(400, "XP insuficiente para comprar Streak Freeze. Custo: 150 XP.")
...
class XPService:
    ...
    def get_xp(user: User) -> XPOut:
        ...
        progress = max(0.0, min(100.0, ((total - current_base) / 1000.0) * 100)) if total >= current_base else 0.0
    ...
    @classmethod
    def award_xp(cls, user: User, amount: int, reason: str = "Atividade concluída") -> XPOut:
        xp_data = user.xp_data if isinstance(user.xp_data, dict) else {}
...
class GoalService:
    ...
    def create_goal(user: User, data: GoalInput) -> GoalOut:
        goals_data = list(user.study_goals or []) if isinstance(user.study_goals, list) else []
    ...
    def delete_goal(user: User, goal_id: str) -> dict:
        goals_data = list(user.study_goals or []) if isinstance(user.study_goals, list) else []
        filtered = [g for g in goals_data if isinstance(g, dict) and g.get("id") != goal_id]
...
class OnboardingService:
    ...
    def mark_done(user: User, has_seen: bool = True, initial_level: str = "A1") -> dict:
        ...
        if initial_level and initial_level.upper() in ['A1', 'A2', 'B1', 'B2', 'C1', 'C2']:
...
class ProgressReportService:
    ...
    def get_weekly_report(user: User) -> dict:
        ...
        msgs = list(Message.objects.filter(username=user.username, role='user', created_at__gte=week_ago))
    ...
    def get_monthly_report(user: Optional[User] = None) -> dict:
        ...
        from apps.chat.models import Message
        msgs = list(Message.objects.filter(username=username, role='user', created_at__gte=month_start))
    ...
    def get_fluency_evolution(user: Optional[User] = None) -> dict:
        current_level = (user.level if user and isinstance(user, User) else "A1") or "A1"
        username = user.username if user and isinstance(user, User) else "aluno"
        ...
        for s in subs:
            ...
            date_str = s.created_at.strftime("%Y-%m-%d") if s.created_at else datetime.now().strftime("%Y-%m-%d")
...
class WeeklyPlanService:
    ...
    def get_weekly_plan(user: Optional[User] = None) -> dict:
        if user and isinstance(user, User) and isinstance(user.weekly_plan, dict) and user.weekly_plan:
        ...
        return {
            ...
            "level": level,
            "days": [
                {"day": 1, "title": "Speaking & Daily Routine", "status": "completed", "xp": 20},
                {"day": 2, "title": "Vocabulary Flashcards: Shopping & Clothes", "status": "available", "xp": 15},
                {"day": 3, "title": "Grammar: Present Simple vs Continuous", "status": "locked", "xp": 25},
                {"day": 4, "title": "Listening Podcast: British Accent Training", "status": "locked", "xp": 20},
                {"day": 5, "title": "Pronunciation Drill with Teacher Tati", "status": "locked", "xp": 30},
                {"day": 6, "title": "Interactive Wordwall Quiz", "status": "locked", "xp": 25},
                {"day": 7, "title": "Weekly Evolution Review", "status": "locked", "xp": 40},
            ]
        }
...
class AvatarService:
    ...
    def get_frames(cls) -> dict:
        ...
        frames = {
            key: (cls._load_frame_b64(fname) or "") for key, fname in cls._FRAME_FILES.items()
        }
