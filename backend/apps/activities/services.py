...
class TrophyService:
    SYSTEM_TROPHIES = [
        ...
        },
        {
            ...
            "description": "Completed 50 vocabulary exercises.",
            "icon": "📖",
            "category": "vocabulary",
            "requirement_type": "vocabulary",
            "requirement_value": 50,
        ...
        },
        {
            "id": "listening-50",
            "name": "Audio Master",
            "description": "Completed 50 listening activities or podcasts.",
            "icon": "🎧",
            "category": "listening",
            "requirement_type": "listening",
            "requirement_value": 50,
    ...
    @classmethod
    def get_unlocked_trophies_count(cls, user: User) -> tuple[int, int]:
        """
        Retorna (troféus_desbloqueados, total_troféus) para o usuário.
        """
        try:
            achievements = cls.get_my_achievements(user)
            unlocked = sum(1 for t in achievements if t.unlocked)
            return unlocked, len(cls.SYSTEM_TROPHIES)
        except Exception as e:
            logger.warning(f"Error calculating trophies count in TrophyService: {e}")
            from apps.activities.models import UserTrophy
            unlocked = UserTrophy.objects.filter(username=getattr(user, "username", "")).count()
            return unlocked, len(cls.SYSTEM_TROPHIES)
