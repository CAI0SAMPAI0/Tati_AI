...
class SecureDocumentService:
    ...
    def _set_processing_status(
        ...
        try:
            self.db.table("premium_content").update(payload).eq(
                "id", content_id
            ).execute()
            try:
                ...
                if upstash_service._ensure_connected() and upstash_service._redis:
                    ...
                    logging.info(
                        f"[Upstash] Caches do catálogo invalidados pós-status: {status}"
                    )
            except Exception as cache_err:
                logging.info(
                    f"[Upstash] Erro ao invalidar cache pós-status: {cache_err}"
                )
        except Exception as e:
            logging.info(f"[SecureDoc] Erro ao atualizar status: {e}")

    def upload_to_drive(self, local_path: str, filename: str) -> str | None:
        if not self.drive_service or not self.drive_folder_id:
            logging.info("[SecureDoc] Drive Service ou Folder ID não configurados.")
            return None
    ...
    def process_pdf_to_images(self, pdf_path: str, output_dir: str) -> list[str]:
        try:
            ...
            logging.info(f"[SecureDoc] Erro ao converter PDF em imagens: {e}")
            return []

    def upload_preview(self, first_page_path: str, content_id: str) -> str | None:
    ...
    def upload_pages_to_supabase(
        self, image_paths: list[str], content_id: str
    ) -> list[str]:
    ...
    def secure_process_document(
        ...
        self.db.table("premium_content").update(update_payload).eq(
            "id", content_id
        ).execute()
        ...
        try:
            ...
            if upstash_service._ensure_connected() and upstash_service._redis:
                ...
                upstash_service._redis.delete("hub:active_contents")
                logging.info(
                    "[Upstash] Caches do catálogo invalidados pós-processamento completo"
                )
        except Exception as cache_err:
            logging.info(
                f"[Upstash] Erro ao invalidar cache pós-processamento completo: {cache_err}"
            )
...
def public_preview_url(preview_path: str | None) -> str | None:
