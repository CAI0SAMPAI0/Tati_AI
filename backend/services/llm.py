...
# Mensagem no formato interno:  {"role": "user"|"assistant", "content": str}
Message = dict
...
async def transcribe_audio(audio_bytes: bytes, filename: str = "temp.wav") -> str:
    ...
    # Groq espera um objeto de arquivo. Usamos BytesIO.
    audio_file = io.BytesIO(audio_bytes)
    audio_file.name = filename # Necessário para o client identificar o formato
...
async def stream_claude(system: str, history: list[Message]) -> AsyncIterator[str]:
    ...
    # Converte histórico para o formato do Anthropic
    # (Claude não aceita system message no histórico, deve ser passada à parte)
    formatted_history = []
    for m in history:
...
async def stream_groq(system: str, history: list[Message]) -> AsyncIterator[str]:
    ...
    # Converte histórico para o formato do Groq (que é igual ao da OpenAI)
