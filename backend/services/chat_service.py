...
from services.llm import stream_llm, text_to_speech, transcribe_audio
from services.history import save_message, load_history, auto_title
from services.prompt_builder import UserProfile, build_effective_prompt
from services.rag_search import obter_contexto_rag, RAGResult
...
class ChatService:
    ...
    async def process_chat_message(
        ...
        asyncio.create_task(
            self._post_response_tasks(username, full_response, websocket, conv_id, is_pdf_generation)
        )
    ...
    async def _post_response_tasks(
        self, username: str, full_response: str, websocket: WebSocket, conv_id: str, is_pdf_generation: bool = False
    ):
        ...
        try:
            ...
            from services.semantic_judge import semantic_judge
            
            # Executa tarefas de background de forma assíncrona
            asyncio.create_task(error_log_service.extract_and_log_errors(username, tts_text, full_response))
            asyncio.create_task(semantic_judge.check_topics_completion(username, f"User: {tts_text}\nTati: {full_response}"))

            await self._check_auto_exercise(username, full_response, websocket)
        except Exception as e:
