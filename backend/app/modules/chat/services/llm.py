...
async def groq_chat(
    ...
    try:
        ...
        if "rate_limit" in str(e).lower() and model == "llama-3.3-70b-versatile":
            ...
            return await _groq_chat_attempt(
...
async def search_image_on_internet(query: str) -> str:
    ...
    try:
        ...
        try:
            logging.info(f"[LLM] Requesting Groq to optimize query for: {query}")
            optimized_query = await groq_chat(
                [
                    {
                        "role": "system",
                        "content": "Generate a short, 3-4 word English search query to find a clear educational image for the following term. Output ONLY the query string, no quotes or explanations.",
                    },
            ...
            logging.info(f"[LLM] Optimized image search query: {optimized_query}")
            ...
            for key in keys:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logging.info(f"[LLM] Tavily found {len(images)} images")
                        if images:
                        ...
                        logging.info(f"[LLM] Tavily API error ({
                                response.status_code}): {
                                response.text}")
                except Exception as inner_e:
                    logging.info(f"[LLM] Erro com chave Tavily: {inner_e}")
                    continue

        return ""
    except Exception as e:
        logging.info(f"[LLM] Erro crítico ao buscar imagem na internet: {e}")
        return ""
...
def preprocess_image_with_opencv(image_bytes: bytes) -> bytes:
    ...
    try:
        ...
        if img is None:
            logging.info(
                "[OpenCV Preprocess] Não foi possível decodificar a imagem com OpenCV."
            )
            return image_bytes
        ...
        if max(h, w) > max_dim:
            ...
            logging.info(
                f"[OpenCV Preprocess] Imagem redimensionada de {w}x{h} para {new_w}x{new_h}"
            )
