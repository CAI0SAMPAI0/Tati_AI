...
def _render_notification_copy(
    ...
    if title == "notif.streak_reminder.title":
        localized_title = (
            "Não perca sua sequência!" if is_pt else "Do not break your streak!"
        )
    elif title == "notif.streak_milestone.title":
        localized_title = (
            "Marcos de sequência desbloqueado!"
            if is_pt
            else "Streak milestone unlocked!"
        )
    elif title == "notif.streak_broken.title":
        localized_title = (
            "Sua sequência foi perdida" if is_pt else "Your streak was broken"
        )
    elif title == "notif.trophy_earned.title":
        localized_title = (
            "Novo troféu desbloqueado!" if is_pt else "New trophy unlocked!"
        )
    else:
    ...
    if message == "notif.streak_reminder.message":
        ...
        localized_message = (
            (f"Você ganhou o troféu: {trophy_name}")
            if is_pt
            else f"You earned the trophy: {trophy_name}"
        )
    else:
...
def create_notification(
    ...
    if send_push:
        try:
            send_push_to_user(
                username, title=rendered_title, body=rendered_body, url=push_url
            )
        except Exception as exc:
...
def _has_notification_today(
    username: str, notif_category: str, now_utc: datetime
) -> bool:
    db = get_client()
...
def notify_trophy_earned(
    username: str, trophy_name: str, trophy_icon: str = "🏆"
) -> None:
    create_notification(
        username=username,
        category="trophy",
...
def notify_correction(
    ...
    score_msg = (
        "Excellent! 🎉"
        if score >= 90
        else "Good job! 👍" if score >= 70 else "Keep practicing! 💪"
    )
...
def notify_new_activity(
    ...
    try:
        ...
        logging.info(f"[Notif] Erro ao enviar e-mail de nova atividade: {exc}")
        ...
        for u in users:
            ...
            if send_email and u.get("email"):
                try:
                    ...
                    EmailSender().send_new_activity_email(
                        ...
                        u.get("name", username),
                        message,
                        f"https://tati-ai.vercel.app{url}",
                    )
                except BaseException:
                    pass
            ...
            try:
                ...
                if loop and loop.is_running():
                    loop.create_task(
                        WahaService.send_message(
                            recipient_username=username, text=whatsapp_text, db=db
                        )
                    )
                else:
                    asyncio.run(
                ...
                logging.info(
                    f"[Notif] Erro ao enviar WhatsApp em notify_all_students para {username}: {wexc}"
                )
...
def notify_ai_generation(
    username: str, title: str, message: str, url: str = "/admin"
) -> None:
    create_notification(
        username=username,
...
def dispatch_streak_engagement_notifications(
    mode: str = "all", now_utc: datetime | None = None
) -> dict[str, int]:
    ...
    try:
        ...
        logging.info(f"[Notif] Falha ao carregar usuários para job: {exc}")
        return summary
