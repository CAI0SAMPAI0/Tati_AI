...
STREAK_MILESTONE_MESSAGES = {
    ...
    3: (
        "3-day streak! 🔥",
        "Three days in a row — you're building a habit. Keep it up!",
    ),
    7: (
        "One week streak! 🏆",
        "7 days of consistent practice! You're officially on a roll.",
    ),
    14: (
        "2-week warrior! ⚡",
        "14 days straight! Your English is getting stronger every day.",
    ),
    30: (
        "30-day legend! 🥇",
        "One full month of practice! Tati is seriously impressed.",
    ),
    60: (
        "60 days! You're unstoppable 🚀",
        "Two months of dedication. You're in the top tier of learners!",
    ),
    100: (
        "100-DAY STREAK! 🎉",
        "ONE HUNDRED DAYS. This is extraordinary. You should be very proud!",
    ),
    365: (
        "365 days! One full year! 🌟",
        "A FULL YEAR of daily practice. You are an absolute legend.",
    ),
}
...
class NotificationScheduler:
    ...
    async def _job_streak_reminders(self):
        ...
        def _fetch():
            return (
                ...
                .select("username, name, role, streak_data")
                .execute()
                .data
                or []
            )
        ...
        for row in users:
            ...
            await self._dispatch(
                username, title_tpl, body, category="streak_reminder", url="/chat"
            )
            count += 1
    ...
    async def _job_broken_streaks(self):
        ...
        for row in users:
            ...
            if freeze_applied:
                ...
                body = (
                    f"Your {current_streak}-day streak is safe. Keep learning tomorrow!"
                )
                await self._dispatch(
                    username, title, body, category="streak_frozen", url="/chat"
                )
                continue
            ...
            body = body_tpl.format(streak=current_streak)
            await self._dispatch(
                username, title_tpl, body, category="streak_broken", url="/chat"
            )
    ...
    def _reset_streak(self, username: str):
        try:
            ...
            logging.info(f"[Scheduler] Erro ao resetar streak de {username}: {e}")

    def _count_notifications_today(
        self, username: str, category: str, now: datetime
    ) -> int:
    ...
    async def check_user_inactivity(self):
        ...
        def _fetch_users_and_activity():
            try:
                users = (
                    self._db.table("users")
                    .select("username, name, role, weekly_plan")
                    .execute()
                    .data
                    or []
                )
        ...
        for row in users:
            ...
            await self._dispatch(
                username, title, body, category="retention", url="/chat"
            )
            count += 1

        logging.info(f"[Scheduler] Inatividade: {count} notificações enviadas")
    ...
    async def send_weekly_progress_reports(self):
        ...
        def _fetch_users():
            try:
                return (
                    self._db.table("users")
                    .select("username, email, name, role, profile")
        ...
        for student in students:
            ...
            try:
                ...
                if already_sent_res.data:
                    logging.info(
                        f"[Scheduler] Relatório semanal já foi enviado para {username} nos últimos 6 dias. Pulando."
                    )
                    continue
            except Exception as e:
                logging.error(
                    f"[Scheduler] Erro ao checar envio anterior de relatório para {username}: {e}"
                )
            ...
            if not email and not responsible_email:
                continue
            try:
                pdf_path = await progress_report_service.generate_student_report(
                    username, lang="en-US"
                )
                ...
                try:
                    ...
                    logging.error(
                        f"[Scheduler] Erro ao registrar notificação de weekly_report para {username}: {e}"
                    )
            except Exception as e:
                logging.info(f"[Scheduler] Erro no relatório de {username}: {e}")

    async def _dispatch(
        self, username: str, title: str, body: str, category: str, url: str = "/"
    ):
        try:
            ...
            logging.info(f"[Scheduler] Erro ao despachar para {username}: {e}")
