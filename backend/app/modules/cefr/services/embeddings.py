    ...
    def generate_and_save_embeddings(
        ...
        logging.info(
            f"Gerando embeddings para {
                len(chunks)} chunks do arquivo {source_file}...")

        # Gera os vetores usando o endpoint do HuggingFace
        # O langchain_huggingface cuida do batching se necessário, mas
        # para chunks pequenos geralmente é tranquilo
        vectors = embeddings_model.embed_documents(chunks)

        logging.info(
            f"Salvando {
                len(vectors)} vetores no Supabase (cefr_documents)...")
        saved_count = 0
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                res = client.table(
                    'cefr_documents').insert(data).execute()
                if res.data:
                    saved_count += 1
            except Exception as e:
                logging.info(
                    f"[EmbeddingsService] Erro ao salvar chunk {i} do arquivo {source_file}: {e}")

        return saved_count

    @staticmethod
    def search_similar_documents(
            query: str, level: str = None, top_k: int = 3) -> List[Dict[str, Any]]:
        """
        ...
        # Parâmetros para a função RPC do Supabase que faz a busca por
        # similaridade
        params = {
        ...
        try:
            # Assumindo que criaremos uma função `match_cefr_documents`
            # no Supabase
            res = client.rpc('match_cefr_documents', params).execute()
            return res.data
        except Exception as e:
            logging.info(
                f"[EmbeddingsService] Erro ao buscar similaridade: {e}")
            return []
