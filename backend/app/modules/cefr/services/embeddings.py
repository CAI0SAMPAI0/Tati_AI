...
class EmbeddingsService:
    ...
    def get_embeddings_model():
        ...
        if prefer_remote:
            logger.info(
                "[EmbeddingsService] Using remote HF endpoint (prefer_remote=true)."
            )
    ...
    def _embed_documents_safe(texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            logger.error(
                ...
                "Falling back to local embeddings model.",
                e,
            )
    ...
    def generate_and_save_embeddings(
        ...
        logging.info(
            f"Generating embeddings for {len(chunks)} chunks of file {source_file}..."
        )
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                ...
                logging.error(
                    f"[EmbeddingsService] Error saving chunk {i} of file {source_file}: {e}"
                )
    ...
    def search_similar_documents(
        query: str, level: str = None, top_k: int = 3, reference_ids: list[str] = None
    ) -> list[dict[str, Any]]:
        """
        ...
        if reference_ids:
            try:
                ...
                logging.error(
                    f"[EmbeddingsService] Error fetching reference paths: {ref_err}"
                )
