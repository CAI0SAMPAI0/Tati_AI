...
class CEFRScheduler:
    ...
    def start(self):
        ...
        logging.info(
            "[CEFR Scheduler] Job semanal de geração autônoma configurado (Segunda 03:00).")
    ...
    async def job_generate_weekly_content(self):
        ...
        """
        logging.info(
            "[CEFR Scheduler] Iniciando geração semanal de conteúdo CEFR...")

        try:
            # 1. Descobrir quais níveis têm materiais indexados
            res = self.client.table(
                'cefr_documents').select('level').execute()
            if not res.data:
                logging.info(
                    "[CEFR Scheduler] Nenhum material CEFR indexado. Cancelando geração.")
                return
            ...
            logging.info(
                f"[CEFR Scheduler] Níveis encontrados para geração: {available_levels}")

            # 2. Para cada nível, sorteia um tópico e gera conteúdo
            for level in available_levels:
                topic = random.choice(EVERYDAY_TOPICS)
                logging.info(
                    f"[CEFR Scheduler] Gerando conteúdo para {level} sobre '{topic}'...")
                ...
                if flashcards:
                    ...
                    for card in flashcards:
                        ...
                        insert_res = self.client.table(
                            "cefr_flashcards").insert(data).execute()
                        if insert_res.data:
                            saved_cards.extend(insert_res.data)
                    logging.info(
                        f"[CEFR Scheduler] Salvos {
                            len(saved_cards)} flashcards para {level}.")
                ...
                if exercises:
                    ...
                    for ex in exercises:
                        ...
                        insert_res = self.client.table(
                            "cefr_exercises").insert(data).execute()
                        if insert_res.data:
                            saved_exercises.extend(insert_res.data)
                    logging.info(
                        f"[CEFR Scheduler] Salvos {
                            len(saved_exercises)} exercícios para {level}.")

            logging.info(
                "[CEFR Scheduler] Geração semanal concluída com sucesso!")

        except Exception as e:
            logging.info(
                f"[CEFR Scheduler] Erro durante a geração semanal: {e}")
