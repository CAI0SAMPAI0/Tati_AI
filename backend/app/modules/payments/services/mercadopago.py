    ...
    async def _post(self, path: str, payload: dict) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                print(f"[MercadoPago] Erro API status={exc.response.status_code}: {error}")
                raise RuntimeError(f"Erro Mercado Pago {exc.response.status_code}: {error}")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                try:
                    ...
                    error = exc.response.text
                print(f"[MercadoPago] Erro API GET status={exc.response.status_code}: {error}")
                raise RuntimeError(f"Erro Mercado Pago {exc.response.status_code}: {error}")

    async def pay_with_pix(self, amount: float, description: str, payer: dict, external_reference: str) -> dict:
        payload = {
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                try:
                    ...
                    error = exc.response.text
                print(f"[MercadoPago] Erro API PUT status={exc.response.status_code}: {error}")
                raise RuntimeError(f"Erro Mercado Pago {exc.response.status_code}: {error}")
