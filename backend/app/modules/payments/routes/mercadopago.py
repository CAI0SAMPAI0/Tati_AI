        ...
        for attempt in range(3):
            try:
                ...
                if "404" in str(e) and attempt < 2:
                    _mp_log(
                        f"Payment {payment_id} not found (404) on attempt {attempt + 1}. Retrying in 1.5s..."
                    )
            ...
            if len(parts) >= 3:
                ...
                if status == "approved":
                    ...
                    try:
                        ...
                        _mp_log(
                            f"premium_purchases confirmed for {username} - {content_id}"
                        )
                    except Exception as e:
                    ...
                    if is_new_hub:
                        try:
                            ...
                            if not (
                                upd_res.data if hasattr(upd_res, "data") else upd_res
                            ):
                                _mp_log(
                                    f"No order found with asaas_id={payment_id}, trying via order_items"
                                )
                                oi_rows = (
                                    db.table("order_items")
                                    .select(
                                        "order_id, orders!inner(id, username, status)"
                                    )
                                ...
                                for oi in oi_rows:
                                    ...
                                    if (
                                        ...
                                        _mp_log(
                                            f"Order {o['id']} confirmed and updated with real MP payment_id"
                                        )
                                        break
                        except Exception as e:
...
async def sandbox_simulate_approve(payment_id: str):
    ...
    if not settings.mp_access_token.startswith("TEST-"):
        raise HTTPException(
            status_code=403, detail="Endpoint disponível apenas em ambiente de teste."
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Pagamento não encontrado no MP: {e}"
        )
    ...
    if len(parts) < 3 or parts[0] not in ("HUB", "PREMIUM"):
        raise HTTPException(
            status_code=400, detail=f"external_reference inválido: {ext_ref}"
        )
    ...
    _mp_log(
        f"[SimulateApprove] Pagamento {payment_id} aprovado manualmente para {username} - {content_id}"
    )
    return {
        "ok": True,
...
async def get_payment_status(
    ...
    try:
        ...
        logging.error(
            f"[MercadoPago][get_payment_status] Erro ao buscar status de pagamento para {username}: {e}"
        )
