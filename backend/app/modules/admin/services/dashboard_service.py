...
class DashboardService:
    ...
    async def get_quick_stats(self) -> dict[str, Any]:
        ...
        def _fetch() -> dict[str, Any]:
            ...
            try:
                ...
                total_students = (
                    res_users.count
                    if res_users.count is not None
                    else len(res_users.data)
                )
                total_buyers = (
                    res_buyers.count
                    if res_buyers.count is not None
                    else len(res_buyers.data)
                )
            except Exception as e:
                logging.info(f"[DashboardService] Erro ao contar usuários: {e}")
                total_students = 0
                total_buyers = 0
            ...
            try:
                ...
                try:
                    ...
                    logging.info(f"[DashboardService] Erro ao contar mensagens: {e}")
                    messages_today = 0
            ...
            try:
                ...
                active_today = len(
                    {r.get("username") for r in active_rows if r.get("username")}
                )
            except Exception:
                active_today = 0
        ...
        try:
            ...
            logging.info(f"[DashboardService] Erro crítico em get_quick_stats: {e}")
            return {
                "total_students": 0,
                "total_messages": 0,
                "active_today": 0,
    ...
    async def get_students_list(self) -> list[dict[str, Any]]:
        ...
        def _fetch() -> list[dict[str, Any]]:
            ...
            for u in users:
                ...
                if last_active_str:
                    try:
                        ...
                        logging.info(
                            f"[DashboardService] Erro ao processar data para {username}: {date_err}"
                        )
            ...
            try:
                processed_users.sort(
                    key=lambda x: parse_dt(x.get("last_active")), reverse=True
                )
            except Exception as sort_err:
                logging.info(f"[DashboardService] Erro ao ordenar alunos: {sort_err}")
        ...
        try:
            ...
            logging.info(f"[DashboardService] Erro em get_students_list: {e}")
            return []
        ...
        def _fetch():
            try:
                ...
                try:
                    ...
                    logging.info(f"[DashboardService] Erro ao buscar níveis: {e}")
        ...
        def _fetch():
            try:
                res = (
                    ...
                    .select(
                        "id, name, description, difficulty, system_prompt, emoji, is_active, created_at"
                    )
                    .order("created_at", desc=True)
                ...
                logging.info(f"[DashboardService] Erro em get_all_simulations: {e}")
                return []
    ...
    async def get_user_stats(self, username: str) -> dict[str, Any]:
        ...
        def _fetch():
            try:
                ...
                level_str = normalize_level(
                    xp_data.get("level") or user_data.get("level") or "A1"
                )
                ...
                fallback_prog = (
                    min(100, int((xp_in_level / xp_needed) * 100))
                    if xp_needed > 0
                    else 0
                )
                ...
                logging.info(f"[DashboardService] Erro em get_user_stats: {e}")
                return {}
    ...
    async def get_student_detail_analytics(self, username: str) -> dict[str, Any]:
        ...
        def _fetch():
            ...
            modules = (
                ...
                .select("id, title, order, flashcards")
                .order("order")
                .execute()
                .data
                or []
            ...
            quizzes = (
                self.db.table("quizzes").select("id, module_id, title").execute().data
                or []
            )
            user_progress = (
            ...
            try:
                ...
                completed_module_ids = {
                    s["module_id"] for s in submissions if s.get("module_id")
                }
            except Exception:
            ...
            for m in modules:
                ...
                if total_quizzes > 0:
                    completed_count = sum(
                        1 for q in mod_quizzes if q["id"] in completed_quiz_ids
                    )
            ...
            try:
                    ...
                    if raw_dt:
                        try:
                            ...
                            if d_str in days_map:
                                days_map[d_str]["study_minutes"] += float(
                                    s.get("duration_minutes") or 0.0
                                )
                ...
                logging.info(
                    f"[DashboardService] Erro ao buscar study_sessions para analíticos: {e}"
                )
            ...
            try:
                ...
                logging.info(
                    f"[DashboardService] Erro ao buscar mensagens para analíticos: {e}"
                )
            ...
            for d_str in sorted(days_map.keys()):
                ...
                d_data["study_minutes"] = round(
                    d_data["study_minutes"] + chat_est_minutes, 1
                )
    ...
    async def nudge_student(
        ...
        try:
            ...
            logging.warning(
                f"[DashboardService] Erro ao enviar push/email/WhatsApp nudge: {push_err}"
            )
    ...
    async def get_recommendations(self, username: str, lang: str = "en-US") -> dict:
        ...
        try:
            ...
            if (
                ...
                and "interests" in result
                and "recommendations" in result
            ):
                return result
        except Exception as e:
            logging.info(
                f"[DashboardService] Erro ao gerar interesses/recomendações via IA: {e}"
            )
    ...
    async def generate_simulation(
        self, topic: str, level: str, instructions: str
    ) -> dict:
        ...
        try:
            ...
            if data:
                ...
                return (
                    res.data[0] if res.data else {"error": "Falha ao salvar no banco"}
                )
    ...
    async def delete_student(self, username: str) -> bool:
        ...
        def _delete():
            ...
            try:
                ...
                logging.info(f"[DashboardService] {
    ...
    async def get_buyers_list(self) -> list[dict[str, Any]]:
        ...
        def _fetch() -> list[dict[str, Any]]:
            try:
                ...
                logging.info(f"[DashboardService] Erro ao buscar buyers: {e}")
                return []
            ...
            try:
                orders_rows = (
                    self.db.table("orders")
                    .select(
                        "id, username, total_amount, status, confirmed_at, created_at, payment_method"
                    )
                    .eq("status", "confirmed")
                    .execute()
                    .data
                ...
                logging.info(f"[DashboardService] Erro ao buscar orders: {e}")
                orders_rows = []
            ...
            for u in users:
                ...
                total_spent = sum(
                    float(o.get("total_amount") or 0) for o in user_orders
                )
                ...
                if user_orders:
                    dates = [
                        o.get("confirmed_at") or o.get("created_at") or ""
        ...
        try:
            ...
            logging.info(f"[DashboardService] Erro em get_buyers_list: {e}")
            return []
