...
def generate_flashcards_task(theme: str, level: str, module_id: str = None, username: str = None):
    from app.modules.activities.services.activity_service import ActivityService
    
    async def _run():
        service = ActivityService()
        res = await service.generate_flashcards(theme=theme, level=level, module_id=module_id)
...
def generate_quiz_task(topic: str, level: str, num_questions: int = 5, username: str = None):
    from app.modules.activities.services.quiz_service import QuizService
    
    async def _run():
        service = QuizService()
        res = await service.generate_dynamic_quiz(topic=topic, level=level, num_questions=num_questions)
        if res and username:
            from app.modules.notifications.services.notifications import notify_ai_generation
            notify_ai_generation(
                username=username,
...
def generate_simulation_task(topic: str, level: str, instructions: str, username: str = None):
    from app.modules.admin.services.dashboard_service import DashboardService
    
    async def _run():
        service = DashboardService()
        res = await service.generate_simulation(topic=topic, level=level, instructions=instructions)
