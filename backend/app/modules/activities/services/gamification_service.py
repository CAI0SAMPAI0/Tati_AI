...
class GamificationService:
        ...
        for attempt in range(retries):
            try:
                ...
                if (
                    ...
                    logging.info(f"[Gamification DB] Connection issue, retrying ({
                            attempt + 1}/{retries})...")
    ...
    async def get_streak_data(self, username: str) -> dict[str, Any]:
        ...
        def _fetch():
            ...
            data["hours_saved"] = (
                sum(s.get("duration_minutes", 0) for s in sessions) / 60.0
            )
    ...
    async def update_streak(self, username: str) -> dict[str, Any]:
        ...
        for k, v in streak_data.items():
            if k not in updated_streak and k not in [
                "trophies_earned",
                "total_questions",
                "hours_saved",
            ]:
    ...
    async def update_user_timezone(self, username: str, tz_name: str) -> None:
        ...
        if current_tz != tz_name:
            ...
            def _update():
                self.db.table("users").update({"streak_data": streak_data}).eq(
                    "username", username
                ).execute()
