...
class ActivityService:
    ...
    async def get_weekly_tasks(self, username: str) -> Dict[str, Any]:
        ...
        def _fetch():
            ...
            all_quizzes = self.db.table('quizzes').select('id, title, description, module_id').eq('is_active', True).execute().data or []
            ...
            print(f"[WeeklyGoalDebug] Pending Quizzes: {len(pending_quizzes)}")
            print(f"[WeeklyGoalDebug] Pending AI Exercises: {len(attempts)}")
            print(f"[WeeklyGoalDebug] Pending Simulations: {len(sims)}")
    ...
    async def generate_flashcards(self, theme: str, level: str, module_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Para não ser muito lento, podemos gerar apenas se o usuário pedir explicitamente via flag
            ...
            def _save_mod():
                payload = {
                    'title': data.get('title', theme.replace('IMG:', '')),
