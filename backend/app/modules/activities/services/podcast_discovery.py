...
async def discover_personalized_podcasts(user_id: str, username: str, user_level: str):
    ...
    try:
        ...
        if (daily_res.count or 0) >= getattr(settings, "video_limit_per_day", 3):
            logging.info(f"[Discovery] Limite diário atingido para {username}")
            return
        ...
        if (weekly_res.count or 0) >= getattr(settings, "video_limit_per_week", 5):
            logging.info(f"[Discovery] Limite semanal atingido para {username}")
            return
    ...
    prompt = (
        ...
        '"duration": "MM:SS", "category": "grammar|vocabulary|speaking|listening", "level": "'
        + user_level
        + '"}]'
    )
    ...
    try:
        ...
        for p in podcasts:
            ...
            if _has_banned_token(p.get("title", ""), p.get("description", "")):
                logging.info(f"[Discovery] Vídeo bloqueado por título: {
                        p.get('title')}")
                continue

            if not _is_duration_valid(p.get("duration", "")):
                logging.info(f"[Discovery] Vídeo bloqueado por duração: {
                        p.get('title')} ({
                        p.get('duration')})")
                continue
        ...
        if not tasks_verify:
            logging.info("[Discovery] Nenhum vídeo válido após filtragem inicial")
            return

        verifications = await asyncio.gather(*tasks_verify)

        existing_podcasts = await run_in_threadpool(
            lambda: db.table("podcasts")
            .select("id")
            .eq("user_id", username)
            .execute()
            .data
        ...
        for i, is_ok in enumerate(verifications):
            ...
            p["level"] = user_level
            p["description"] = (
                p.get("description") or "Short English lesson recommended by Tati AI"
            )
            ...
            await run_in_threadpool(
                lambda _p=p: db.table("podcasts").upsert(_p).execute()
            )
            valid_count += 1

        if valid_count > 0:
            from app.modules.activities.routes.podcasts import (
