...
class QuizService:
    ...
    async def get_quiz(
        self, quiz_id: str, username: str | None = None
    ) -> dict[str, Any] | None:
        ...
        if str(quiz_id).startswith("cefr_"):
            ...
            def _fetch():
                quiz_data = (
                    ...
                    .select(
                        "*, modules(title, description, image_url, youtube_url, spotify_url, file_url)"
                    )
                    .eq("id", quiz_id)
                    .single()
                    .execute()
    ...
    async def evaluate_submission(
        ...
        try:
            ...
            if xp_reward > 0:
                asyncio.create_task(gs.award_xp(username, xp_reward, f"Quiz: {
                            quiz.get(
                                'title',
                                'Practice')}"))
    ...
    async def generate_module_quiz(
        self, title: str, level: str, context: str, num_questions: int = 5
    ) -> dict[str, Any]:
        """Gera um quiz completo para um módulo."""
        return await self.generate_dynamic_quiz(
            f"{title} ({context})", level, num_questions=num_questions
        )

    async def generate_dynamic_quiz(
        self, topic: str, level: str, num_questions: int = 5
    ) -> dict[str, Any]:
