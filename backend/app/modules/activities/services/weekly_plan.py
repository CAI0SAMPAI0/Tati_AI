...
async def get_or_generate_weekly_plan(
        username: str,
        level: str,
        focus: str) -> dict:
...
async def check_plan_progress(username: str) -> dict:
    ...
    try:
        ...
        logging.info(
            f'[WeeklyPlan] Erro ao buscar histórico para progresso: {e}')
        return {}
...
"""
        try:
            ...
            logging.info(
                f'[WeeklyPlan] Erro ao gerar exercícios tipo {ex_type}: {e}')
...
async def _save_as_quiz(
    ...
    try:
        mod_check = (
            db.table('modules').select('id').eq(
                'id', PERSONALIZED_MODULE_ID).execute()
        )
