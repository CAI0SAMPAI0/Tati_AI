...
async def check_plan_progress(username: str) -> dict:
    ...
    try:
        ...
        logging.info(f"[WeeklyPlan] Erro ao buscar histórico para progresso: {e}")
        return {}
...
"""
    try:
        ...
        logging.info(f"[WeeklyPlan] Erro ao checar progresso: {e}")
        return {}
...
async def generate_transition_exercises(username: str) -> dict | None:
    ...
    chosen_types = random.sample(
        _EXERCISE_TYPES, k=random.choice([1, 1, 2])
    )  # favorece 1 tipo
...
"""
        try:
            ...
            logging.info(f"[WeeklyPlan] Erro ao gerar exercícios tipo {ex_type}: {e}")
    ...
    type_labels.get(exercise_type_label, "Practice")
    ...
    # Invalida cache do plano para forçar geração novo na próxima
    # chamada
...
def _fetch_rag_context(query: str) -> str:
    ...
    try:
        from app.modules.chat.services.rag_search import obter_contexto_rag

        result = obter_contexto_rag(query)
        return result.contexto or "No reference material found for these topics."
    except Exception as e:
        logging.info(f"[WeeklyPlan] RAG error: {e}")
        return "No reference material available."


async def _save_as_quiz(
    db, username: str, title: str, description: str, exercises: list[dict]
) -> str | None:
    ...
    try:
        mod_check = (
            db.table("modules").select("id").eq("id", PERSONALIZED_MODULE_ID).execute()
        )
        ...
        logging.info(f"[WeeklyPlan] Aviso módulo: {e}")
    ...
    try:
        ...
        logging.info(
            f"[WeeklyPlan] Quiz de transição gerado: {quiz_id} para {username}"
        )
        return quiz_id

    except Exception as e:
        logging.info(f"[WeeklyPlan] Erro ao salvar quiz: {e}")
        return None
...
async def _generate_plan(username: str, level: str, focus: str) -> dict:
    ...
    try:
        ...
        logging.info(f"[WeeklyPlan] Erro ao buscar histórico: {e}")
        history = ""

    # Busca progresso da semana anterior para adaptar o novo plano
    prev_week_key = f"weekly_plan_progress:{username}:{
        _week_number() - 1}"
    prev_progress = await cache_get(prev_week_key) or {}

    not_done = [
        topic
        for topic, status in prev_progress.items()
        if topic != "overall" and status in ("not_started", "partial")
    ]

    prev_context = ""
    if not_done:
        prev_context = f'\nTopics the student did NOT fully practice last week (prioritize these): {
            ", ".join(not_done)}'
...
Week: {week_start.strftime('%b %d')} - {week_end.strftime('%b %d, %Y')} {prev_context}
...
{"week": "{week_start.strftime('%b %d')} - {week_end.strftime('%b %d')}",
  ...
  "focuses": [
    ...
    try:
        ...
        logging.info(f"[WeeklyPlan] Erro ao gerar: {e}")
        ...
        "focuses": [
            {
                ...
                "why": "Build confidence speaking naturally.",
                "tip": "Try to write 5 sentences today without stopping.",
            },
            {
                "topic": "Vocabulary Expansion",
                "why": "More words = more expression.",
                "tip": "Learn 3 new words and use them in chat.",
            },
            {
                "topic": "Grammar Review",
                "why": "Solid grammar makes you sound fluent.",
                "tip": "Focus on verb tenses in your messages.",
            },
        ],
    }
