...
class PremiumService:
    ...
    async def list_content_for_student(self, username: str) -> list[dict[str, Any]]:
        ...
        def _fetch():
            ...
            purchases = (
                self.db.table("premium_purchases")
                .select("content_id")
    ...
    async def get_content_access(self, content_id: str, username: str) -> str:
        ...
        def _check():
            ...
            if (
                content.get("is_secure")
                and content.get("processing_status") in ("ready", "skipped")
                and content.get("secure_pages")
            ):
                pages = content["secure_pages"]
                secure_urls = []
                for p in pages:
                    res = self.db.storage.from_("hub-secure-pages").create_signed_url(
                        p, 1800
                    )
            ...
            try:
                ...
                res = self.db.storage.from_(self.bucket).create_signed_url(
                    file_path, 900
                )
                return res["signedURL"]
            except Exception as e:
                logging.info(f"[PremiumService] Erro ao gerar Signed URL: {e}")
                raise HTTPException(
                    status_code=500, detail="Erro ao gerar link de acesso."
                )
    ...
    def _validate_create_payload(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if category not in VALID_CATEGORIES:
            raise BusinessLogicError(detail=f'Categoria inválida. Use: {
                    ", ".join(
    ...
    async def create_content(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            content_type in ("file", "pdf")
            and source
            and not str(source).startswith("http")
        ):
            import asyncio
    ...
    async def _trigger_secure_processing(self, content: dict[str, Any]) -> None:
        ...
        def _run():
            try:
                logging.info(f"[PremiumService] Processamento seguro: {content_id}")
                ...
                if not result.get("success"):
                    logging.info(f'[PremiumService] Falha: {
                            result.get("error")}')
            except Exception as e:
                logging.info(f"[PremiumService] Falha no processamento: {e}")
    ...
    async def update_content(
        ...
        if (
            "price" in payload
            and not has_new_prices
            and float(payload.get("price") or 0) <= 0
        ):
            raise BusinessLogicError(detail="Preço deve ser maior que zero.")
    ...
    async def list_public_catalog(self) -> list[dict[str, Any]]:
        ...
        def _fetch():
            ...
            rows = [
                ...
                for r in rows
                if float(
                    r.get("price_students")
                    or r.get("price_buyers")
                    or r.get("price")
                    or 0
                )
                > 0
    ...
    async def get_public_item(self, item_id: str) -> dict[str, Any] | None:
        ...
        def _fetch():
            ...
            has_price = (
                float(
                    item.get("price_students")
                    or item.get("price_buyers")
                    or item.get("price")
                    or 0
                )
                > 0
