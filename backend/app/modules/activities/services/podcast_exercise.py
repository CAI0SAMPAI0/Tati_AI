...
class PodcastExerciseService:
    ...
    async def generate_exercises(
        ...
        logging.info(f"[exercise] transcript segments count: {
                len(segments)}")
        transcript_context = "\n".join(segments)
        logging.info(
        ...
        if not transcript_context.strip():
            transcript_context = f"Topic/Description: {
                podcast.get(
                    'description', '')}"
            logging.info(
                "[exercise] WARNING: sem transcrição, usando description como fallback"
            )
        ...
        try:
            ...
            payload = await groq_chat_json(
                [{"role": "system", "content": system_prompt}], temperature=0.8
            )
            return self._normalize_exercises_payload(
                payload, podcast.get("title", ""), ui_lang, None
            )
        except Exception as exc:
            logging.info(f"Error generating/parsing LLM response for exercises: {exc}")
    ...
    async def _try_fetch_transcript(self, podcast: dict[str, Any]):
        ...
        if not video_id or len(video_id) != 11:
            logging.info(f"[transcript] video_id inválido: {
                    video_id!r}")
        ...
        def _fetch():
            try:
                ...
                logging.info(f"Transcript fetch failed for {video_id}: {e}")
                return []
