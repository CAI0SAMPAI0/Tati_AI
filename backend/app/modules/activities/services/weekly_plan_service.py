...
class WeeklyPlanService:
    ...
    async def get_or_generate_plan(
            self, username: str, level: str, focus: str) -> Dict[str, Any]:
        """
        Retorna o plano atual ou gera um novo se necessário (expirado ou concluído).
        """
        def _fetch():
            res = self.db.table('users').select('weekly_plan').eq(
        ...
        # Verifica se precisa de novo plano (7 dias ou se plano não
        # existe)
        ...
        if not current_plan:
            ...
            created_at = datetime.fromisoformat(
                current_plan.get('created_at', now.isoformat()))
            # Se passou 7 dias ou se todos os tópicos estão feitos
            # (concluído antecipadamente)
            all_done = all(
                t.get('status') == 'completed' for t in current_plan.get(
                    'topics', []))
    ...
    async def _save_plan(self, username: str,
                         plan_data: Dict[str, Any]):
        def _save():
            self.db.table('users').update({'weekly_plan': plan_data}).eq(
