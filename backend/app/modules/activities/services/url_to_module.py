...
class UrlToModuleService:
    ...
    async def generate_from_url(
        self, url: str, username: str, target_level: str = "B1"
    ) -> dict[str, Any]:
        """
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
                ...
                # Limita o texto para não estourar contexto (aprox 2000
                # palavras)
                text = " ".join(text.split()[:2000])
        except Exception as e:
            return {"ok": False, "error": f"Scraping failed: {e!s}"}
        ...
        try:
            ...
            module_data = await groq_chat_json(
                ...
                "contents": [
                    {
                        ...
                        "content": module_data.get("reading_content"),
                        "order": 0,
                    },
                    {
                        "title": "Grammar Insights",
                        "content": module_data.get("grammar_focus"),
                        "order": 1,
                    },
                ],
                "quiz": {
            ...
            logging.info(f"[UrlToModule] Erro: {e}")
