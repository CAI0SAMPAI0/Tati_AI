...
class ModuleService:
    ...
    async def create_module(self, payload: Any) -> str:
        ...
        try:
                    ...
                    .insert(
                        {
                            ...
                            "level": (
                                normalize_level(payload.levels[0])
                                if payload.levels
                                else "A1"
                            ),
            ...
            raise HTTPException(500, f"Erro ao criar módulo: {e!s}")
    ...
    async def update_module(self, module_id: str, payload: Any) -> bool:
        ...
        try:
            update_data = {
                ...
                "ai_prompt": (
                    payload.ai_prompt if hasattr(payload, "ai_prompt") else None
                ),
            }
            ...
            raise HTTPException(500, f"Erro ao atualizar módulo: {
                    e!s}")
    ...
    async def list_for_student(self, user: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for m in modules_data:
            ...
            show = (
                ...
                or any(
                    str(x).lower().strip() in ["all", "todos", "any", "all levels"]
                    for x in lvls_raw
                )
                or str(sing_raw).lower().strip()
                in ["all", "todos", "any", "all levels"]
