...
class VocabularySRSService:
    ...
    async def add_to_srs(
        ...
        def _add():
            ...
            existing = self.db.table('user_vocabulary').select('id').eq(
                'username', username).eq('word', word).execute().data
            if existing:
                return
    ...
    async def record_review(
        ...
        def _update():
            ...
            res = self.db.table('user_vocabulary').select(
                '*').eq('id', entry_id).eq('username', username).single().execute()
            if not res.data:
                return
            ...
            ef = ef + (0.1 - (5 - quality_score) *
                       (0.08 + (5 - quality_score) * 0.02))
            if ef < 1.3:
                ef = 1.3
            ...
            self.db.table('user_vocabulary').update(
