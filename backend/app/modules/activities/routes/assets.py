...
async def flashcard_ai_image(data: dict):
    ...
    try:
        ...
        if not permanent_url:
            raise HTTPException(
                status_code=500,
                detail='Falha ao salvar imagem no Cloudinary')
        return {"url": permanent_url}
    except Exception as e:
        logging.info(f"[Assets] AI image error: {e}")
        raise HTTPException(
            status_code=500, detail=f'Erro ao gerar imagem: {str(e)}')
...
async def flashcard_upload_image(
    ...
    try:
        logging.info(
            f"[Assets] Iniciando upload de arquivo: {
                file.filename} ({
                file.content_type})")
        ...
        if not url:
            logging.info(
                "[Assets] Cloudinary retornou URL vazia para o upload.")
            raise HTTPException(
                status_code=500,
                detail='Falha ao processar imagem no servidor de mídia')

        logging.info(f"[Assets] Upload concluído com sucesso: {url}")
        return {"url": url}
    except Exception as e:
        logging.info(f"[Assets] Erro crítico no upload: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f'Erro interno no upload: {str(e)}')
...
async def flashcard_upload_image_from_url(
    ...
    try:
        ...
        if not permanent_url:
            raise HTTPException(
                status_code=500, detail='Falha ao salvar imagem da URL')
        return {"url": permanent_url}
    except Exception as e:
        logging.info(f"[Assets] URL upload error: {e}")
        raise HTTPException(
            status_code=500, detail=f'Erro ao processar URL: {str(e)}')
