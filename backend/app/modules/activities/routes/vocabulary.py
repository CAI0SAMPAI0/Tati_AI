...
async def record_vocabulary_review(
    entry_id: str, quality_score: int, user: dict = Depends(get_current_user)
):
    ...
    await vocabulary_srs_service.record_review(
        entry_id, quality_score, user["username"]
    )
    return {"ok": True}
...
async def add_word_manually(data: dict, user: dict = Depends(get_current_user)):
    ...
    await vocabulary_srs_service.add_to_srs(
        user["username"], word, data.get("definition", ""), data.get("example", "")
    )
    return {"ok": True}
...
async def update_vocabulary_entry(
    entry_id: str, payload: dict, user: dict = Depends(get_current_user)
):
