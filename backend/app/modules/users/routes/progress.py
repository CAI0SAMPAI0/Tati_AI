...
async def get_ranking_by_level_endpoint(user=Depends(get_current_user)):
...
async def get_recent_errors(user=Depends(get_current_user)):
...
async def download_progress_report(user=Depends(get_current_user), lang: str = "pt-BR"):
    ...
    pdf_path = await progress_report_service.generate_student_report(
        user["username"], lang=lang
    )

    if not os.path.exists(pdf_path):
        raise HTTPException(status_code=500, detail="Erro ao gerar arquivo PDF")

    return FileResponse(path=pdf_path, filename=f"TatiAI_Report_{
            datetime.now().strftime('%Y-%m-%d')}.pdf", media_type="application/pdf")
...
async def get_fluency_evolution(
    ...
    """
    return await service.get_fluency_evolution(user["username"])
