...
def get_goals(username: str) -> dict:
    ...
    if 'daily_minutes' in goal_types:
        try:
            sessions = db.table('study_sessions').select('duration_minutes, created_at').eq('username', username).execute().data or []
            today_date = datetime.now(timezone.utc).date()
            for s in sessions:
                created = s.get('created_at')
                if created:
                    try:
                        dt_date = datetime.fromisoformat(created.replace('Z', '+00:00')).date()
    ...
    if 'daily_messages' in goal_types:
        try:
            res = db.table('messages').select('id', count='exact').eq('username', username).eq('role', 'user').eq('date', today_str).execute()
            daily_messages = res.count if res.count is not None else (len(res.data) if res.data else 0)
        except Exception:
            pass
            
    # Calculate weekly_conversations
    if 'weekly_conversations' in goal_types or 'weekly_conversations' in [g.get('type') for g in goals_list]:
        try:
            res = db.table('conversations').select('id').eq('username', username).gte('created_at', seven_days_ago_str).execute()
            weekly_conversations = len(res.data) if res.data else 0
        except Exception:
            pass
            
    # Calculate weekly_words
    if 'weekly_words' in goal_types or 'weekly_words' in [g.get('type') for g in goals_list]:
        try:
            res = db.table('user_vocabulary').select('id').eq('username', username).gte('created_at', seven_days_ago_str).execute()
    ...
    if changed:
        ...
        try:
            db.table('users').update({'study_goals': goals_data}).eq('username', username).execute()
        except Exception:
            pass
