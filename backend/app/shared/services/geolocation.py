...
async def detect_country_from_ip(ip_address: str) -> str | None:
    """
...
def get_business_days_in_month(
    year: int, month: int, country: str = "BR"
) -> list[date]:
...
def calc_due_date_local(
    reference: date, preferred_day: int = 5, country: str = "BR"
) -> date:
    """
...
async def get_timezone_from_ip(ip_address: str) -> str | None:
...
async def get_user_location_info(client_ip: str) -> dict:
    ...
    return {
        ...
        "next_business_day": next_business_day_local(
            today, country or "BR"
        ).isoformat(),
        "business_days_remaining_this_month": len(
            [
                d
                for d in get_business_days_in_month(
                    today.year, today.month, country or "BR"
                )
                if d > today
            ]
        ),
