...
async def get_ranking_by_level_endpoint(
    ...
    from services.ranking import get_ranking_by_level
...
async def get_weekly_plan(
    ...
    from services.weekly_plan_service import weekly_plan_service
    return await weekly_plan_service.get_or_generate_plan(
        user['username'],
        user.get('level', 'Intermediate'),
        user.get('focus', 'General Conversation'),
    )
...
async def get_plan_progress(
    ...
    return await service.check_plan_progress(user['username'])
...
async def start_plan_transition(
    ...
    result = await service.start_plan_transition(user['username'])
    if not result:
        raise HTTPException(status_code=500, detail='Erro ao gerar exercícios.')
    return result
