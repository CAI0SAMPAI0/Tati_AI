...
export default function SecureDocumentViewer({
  ...
  return (
    <div
      ref={viewerRef}
      className={`secure-viewer relative select-none ${isFullscreen ? 'bg-background overflow-y-auto p-4' : ''}`}
      ...
      <div className="space-y-6">
        {access.pages?.map((url, index) => {
          ...
          return (
            <figure
              key={`${url}-${index}`}
              className={`relative overflow-hidden rounded-hub border border-line bg-white shadow-sm mx-auto ${isFullscreen ? 'max-w-5xl my-4 shadow-lg' : 'w-full'
                }`}
            >
              <div
              ...
              >
                {/* Imagem de Fundo (Página do Documento) */}
                <Image
                  src={safeUrl}
                  alt={`Página ${index + 1}`}
                  fill
                  ...
                  unoptimized
                  priority={index < 4} // Pré-carrega as 4 primeiras com máxima prioridade
                  onLoad={(e) => {
                  ...
                  onError={(e) => {
                    const img = e.target as HTMLImageElement;
                    // Se falhar, tenta recarregar com cache-busting uma vez
