...
let pendingFile    = null;   // { name, b64, size, type }
let streamBuffer   = '';
...
function triggerFileInput() {
  ...
  input.onchange = async () => {
    ...
    if (file.size > MAX) { appendErrorMsg('Arquivo muito grande (máx 10MB).'); return; }

    const reader = new FileReader();
    reader.onload = () => {
      const b64 = reader.result.split(',')[1];
      pendingFile = { name: file.name, b64, size: file.size, type: file.type };
...
function showFilePreview(name, size) {
  ...
  bar.innerHTML = `
    <div class="file-preview-inner">
      <div class="file-preview-icon">${getFileIcon(name)}</div>
      <div class="file-preview-info">
        <span class="file-preview-name">${escHtml(name)}</span>
        <span class="file-preview-size">${formatFileSize(size)}</span>
      </div>
      <button class="file-preview-remove" title="Remover arquivo" onclick="removePendingFile()">✕</button>
    </div>
    <p class="file-preview-hint">Arquivo será enviado junto com sua próxima mensagem</p>
  `;
    ...
    <div class="message-body">
      <div class="message-bubble file-bubble">
        <div class="file-attach-preview">
          <span class="file-attach-icon">${getFileIcon(name)}</span>
...
function clearMessages() {
  ...
  welcome.style.display = 'flex'; typing.style.display = 'none';
