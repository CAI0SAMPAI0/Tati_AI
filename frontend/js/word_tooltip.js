(function () {
  ...
  const API = (location.hostname === 'localhost' || location.hostname === '127.0.0.1')
    ? 'http://localhost:8000'
    : 'https://tatiai-production.up.railway.app';
  ...
  async function translateWord(word) {
    ...
    // Palavras iguais em PT (empréstimos do inglês)
    const loanwords = ['feedback', 'online', 'software', 'hardware', 'download',
      'upload', 'email', 'internet', 'marketing', 'meeting'];
    if (loanwords.includes(word.toLowerCase())) {
      cache[key] = word; // mesmo em português
      return word;
    }

    try {
      const res = await fetch(
        `https://api.mymemory.translated.net/get?q=${encodeURIComponent(word)}&langpair=en|pt-BR&de=tati@app.com`
      );
      const data = await res.json();
      const t = data?.responseData?.translatedText;
      const valid = t
        && t.toLowerCase() !== word.toLowerCase()
        && !t.toUpperCase().startsWith('MYMEMORY')
        && !t.toUpperCase().startsWith('PLEASE')
        && data?.responseStatus === 200;
      cache[key] = valid ? t : null;
      return cache[key];
    } catch {
      cache[key] = null;
      return null;
  ...
  async function showTooltip(word, x, y) {
    ...
    // Translation
    tooltip.querySelector('.wt-trans-text').textContent = translation || '(palavra igual em português)';
